...
% Generate URLs.txt and Stockcode.mat
    ...
    save('Stockcode.mat','Stockcode','Stockname','StockcodeUrl','startIdx','endIdx','SperPage');
    temp_env = load('Stockcode.mat');
    
    %´´Òµ°å
    [~, Idx, Stockcode,Stockname] = ScanForValidCode_CYB(PackageN);
    StockN = length(Idx);
    PageN = floor(StockN / PackageN) + 1;
    urls = cell(PageN,1);
    SperPage = ones(PageN,1) * ceil(StockN / PageN);
    temps = 1;
    fid = fopen('URLs.txt','a');
    StockcodeUrl = cell(0);
    for j = 1:PageN
        tempe = min(temps + SperPage(1) - 1,StockN);   
        StockcodeUrl{j} = Stockcode(temps:tempe,:);
    ...
    % merge
    Stockcode = [temp_env.Stockcode;Stockcode];
    Stockname = [temp_env.Stockname;Stockname];    
    StockcodeUrl = [temp_env.StockcodeUrl,StockcodeUrl];
    
    SperPage = [temp_env.SperPage;SperPage];
    startIdx = cumsum([1;SperPage(1:end-1)]);
    endIdx = cumsum(SperPage);
...
function [pointers,idx,stockcode,stockname] = ScanForValidCode_CYB(packsize)
% Scan which stocks data can be gathered.
% url_size: max number of stock codes can be sent in one url
% from 0:maxN-1 + sz300000
    n = 0;
    maxN = 1000;  % 0 : max-1
    pointers = zeros(maxN,1);
    stockname = cell(maxN,1);
    ...
    idx = find(pointers == 1)-1;
    stockcode = reshape(sprintf('sz30%04d',idx),8,[])';
    pointers = pointers(1:idx(end)+1);
    stockname = stockname(pointers==1);
end